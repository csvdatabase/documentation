...
Request bodies use ordinary JSON values. `bigint` and exact `numeric` values use decimal strings so they are not rounded by JSON parsers. Dates and timestamps use their CSDB string forms. JSON-typed columns contain normal nested JSON values. See [Types](../reference/types) for the complete mapping.
